...
def parse_args():
    ...
    # SLAM ARGS
    parser.add_argument(
        "--parallel_run", action="store_true", help="Whether to run in parallel"
    )
    parser.add_argument(
        "--multi_gpu",
        action="store_true",
        help="Whether to run with multiple (two) GPUs",
    )
    parser.add_argument(
        "--initial_k", type=int, help="Initial frame to parse in the dataset", default=0
    )
    parser.add_argument(
        "--final_k",
        type=int,
        help="Final frame to parse in the dataset, -1 is all.",
        default=-1,
    )
    parser.add_argument(
        "--img_stride",
        type=int,
        help="Number of frames to skip when parsing the dataset",
        default=1,
    )
    parser.add_argument("--stereo", action="store_true", help="Use stereo images")
    parser.add_argument(
        "--weights", default="droid.pth", help="Path to the weights file"
    )
    parser.add_argument(
        "--buffer", type=int, default=512, help="Number of keyframes to keep"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="euroc",
        choices=["euroc", "nerf", "replica", "real", "gopro"],
    ...
    parser.add_argument(
        ...
        type=str,
        default="ours",
        choices=["no_depth", "raw", "ours", "ours_w_thresh"],
    )
    ...
    parser.add_argument(
        "--gui",
        action="store_true",
        help="Run O3D Gui, use when volume='tsdf'or'sigma'.",
    )
    parser.add_argument(
        "--width",
        "--screenshot_w",
...
def run(args):
    ...
    if fusion:
        ...
        if (args.fusion == "nerf" and not slam) or (
            args.fusion != "nerf" and args.eval
        ):
    ...
    if gui:
        gui_module = GuiModule(
            "Open3DGui", args, device=cuda_slam
        )  # don't use cuda:1, o3d doesn't work...
    ...
    if args.parallel_run:
        ...
        if fusion:
            fusion_thread = Process(
                target=fusion_module.spin
            )  # FUSION NEEDS TO BE IN A PROCESS
        # if slam: slam_thread = Process(target=slam_module.spin, args=())
        if gui:
            gui_thread = Process(target=gui_module.spin, args=())
        ...
        if gui:
            gui_module.shutdown_module()
        if fusion:
            fusion_module.shutdown_module()
        data_provider_module.shutdown_module()

        if gui:
            gui_thread.terminate()  # violent, should be join()
        # if slam: slam_thread.terminate() # violent, should be join()
        if fusion:
            fusion_thread.terminate()  # violent, should be join()
